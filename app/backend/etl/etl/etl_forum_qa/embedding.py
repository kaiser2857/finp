...
def start_embedding_forum_qa(context: EtlContext) -> None:
    """Start the embedding process for forum QA data.
    ...
    input_base_path = os.path.join(
        root_path, f"etl_forum_qa/.temp/outputs_generate_qa/{product}"
    )
    output_base_path = os.path.join(
        root_path, f"etl_forum_qa/.temp/outputs_embedding/{product}"
    )
