...
class Config:
    ...
    def from_environment(cls, environment: str) -> "Config":
        ...
        return cls(
            ...
            mutlimodel=MutilModelConfig(
                api_key=config_raw["mutlimodel"]["api_key"],
                api_base=config_raw["mutlimodel"]["api_base"],
                model_name=config_raw["mutlimodel"]["model_name"],
                max_rpm=config_raw["mutlimodel"].get("max_rpm", 100),
            ), 
...
def get_config() -> Config:
    ...
    load_dotenv()
    environment = os.getenv("RAG_ENV")
    if not environment:
        raise ValueError("RAG_ENV environment variable is not set")
